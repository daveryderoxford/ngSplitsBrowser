...
export const mappedConverter = <T>(): FirestoreDataConverter<T> => ({
   toFirestore(data: PartialWithFieldValue<T>, options?: SetOptions): DocumentData {
      ...
      return toDbModel(data, partialObject);
   },
   fromFirestore: (snap: QueryDocumentSnapshot): T => {
      const data = snap.data();
      return toAppModel<T>(data);
   },
});
 ...
 */
export function toDbModel<T>(data: PartialWithFieldValue<T>, partialUpdate: boolean): DocumentData {
