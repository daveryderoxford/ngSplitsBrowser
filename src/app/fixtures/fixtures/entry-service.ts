...
export class EntryService {
   ...
   async addEntryDetails( details: Partial<FixtureEntryDetails> ): Promise<FixtureEntryDetails> {
      ...
      await this.fs.doc( "entries/" + details.fixtureId ).set( details);
  //    return this.fs.doc<FixtureEntryDetails>( "entries/" + details.fixtureId ).get().pipe(
   //      map(snap => snap.data())
  //    ).toPromise();
   ...
   async updateEntryDetails( details: FixtureEntryDetails ) {
      return this.fs.doc( "entries/" + details.fixtureId ).update( details );
   }
   ...
   //   entry.Id =
      ...
      await this.fs.collection( "entries/" + fixture.fixtureId ).add(entry);
      return Promise.resolve(<Entry>entry);
   }

   /** Update entry details */
   async updateEntry( fixture: FixtureEntryDetails, entry: Entry): Promise<void> {
      return this.fs.collection( "entries/" + fixture.fixtureId ).doc( entry.id).update( entry );
   }

   /** Delete an entry */
   async deleteEntry( fixture: FixtureEntryDetails, entry: Entry ) {
      return this.fs.collection( "entries/" + fixture.fixtureId ).doc( entry.id ).delete();
   }
   ...
   getEntries$( fixture: FixtureEntryDetails ): Observable<Entry[]> {
      return this.fs.collection<Entry>( "entries/" + fixture.fixtureId ).valueChanges();
   }
}
