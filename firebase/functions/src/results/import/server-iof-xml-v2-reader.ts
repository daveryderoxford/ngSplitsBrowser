...
export class Version2Reader implements IOFXMLReader {
    ...
    */
    checkVersion(rootElement: XmlQuery) {
        const iofVersionElement = rootElement.find("> IOFVersion");
    ...
    */
    readClassName(classResultElement: XmlQuery): string {
        return classResultElement.find("> ClassShortName").text();
    }
    ...
    */
    readCourseFromClass(classResultElement: XmlQuery, warnings: string[]): CourseDeatils {
        ...
        const courseName = classResultElement.find("> ClassShortName").text();

        const firstResult = classResultElement.find("> PersonResult > Result").first();
        let length = null;

        if (firstResult.length > 0) {
            const lengthElement = firstResult.find("> CourseLength");
    ...
    */
    getCompetitorNameElement(element: XmlQuery) {
        return element.find("> Person > PersonName");
    }
    ...
    */
    readDateOfBirth(element: XmlQuery): number {
        // eslint-disable-next-line radix
        return Number.parseInt(element.find("> Person > BirthDate > Date").text());
    }
    ...
    *     null if not found.
    */
    readStartTime(resultElement: XmlQuery) {
        const startTimeStr = resultElement.find("> StartTime > Clock").text();
    ...
    */
    getStatus(resultElement: XmlQuery): string {
        const statusElement = resultElement.find("> CompetitorStatus");
    ...
    */
    readSplitTime(splitTimeElement: XmlQuery): {code: string, time: number | null } {
        // IOF v2 allows ControlCode or Control elements.
        let code = splitTimeElement.find("> ControlCode").text();
        if (code === "") {
            code = splitTimeElement.find("> Control > ControlCode").text();
        }
    ...
    */
    readECard(resultElement: XmlQuery): string {
       const card = resultElement.find("> CCard > CCardId").text();
       return card;
   }
}
